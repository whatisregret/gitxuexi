...
def sendRequest(url, method, params=None, data=None,
                headers=None, json=None, cookies=None, timeout=60):
    """
    :param url: 接口请求地址
    ...
    if new_method == 'get':
        ...
        responseResult = requests.get(url=url, params=params, headers=headers, json=json, timeout=timeout)
    elif new_method == "post":
        if json:
            INFO.logger.info("正在发送POST请求，请求地址：{}， 请求参数{}".format(url, json))
